...
check_for_updates() {
    ...
    # Redirect git fetch output to a temporary variable
    ...
    if [ "$LOCAL_COMMIT" != "$REMOTE_COMMIT" ]; then
        ...
        if [ "$DEBUG" != true ]; then
            # Show fetch output only if debug is off and update occurs
            echo "$FETCH_OUTPUT"
        fi
...
start_application() {
    ...
    $START_SCRIPT &  # Start the application in the background
...
stop_application() {
    ...
    if kill -SIGTERM "$APP_PID" 2>/dev/null; then
